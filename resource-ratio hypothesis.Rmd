...
simulate_succession <- function(t, pop, params) {
  with(as.list(c(pop, params)), {
    ...
    dN1 = m1 * (R - R1star) * N1 # estimate the change in the population of each species
   ...
   # cat("Time = ", t, "R = ", R, "\n")
    return(list(c(dN1, dN2, dN3, dN4, dN5), # population number
                R = R, 
                pcg1 = dN1/N1,  # percapita growth rate
                pcg2 = dN2/N2,
                pcg3 = dN3/N3,
                pcg4 = dN4/N4,
                pcg5 = dN5/N5))
...
sucession_solution_long_percapita <- succession_solution %>%
  select(R, pcg1, pcg2, pcg3, pcg4, pcg5) %>%
  gather("pop", "rate",-R)
...
sucession_solution_long <- succession_solution %>%
  select(!c(pcg1, pcg2, pcg3, pcg4, pcg5)) %>%
